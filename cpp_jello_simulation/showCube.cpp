...
/* Renders the plane intersecting with the bounding box.
   The plane parameters are defined in the input world file. */
void showPlane(struct world* jello)
{  

   // Define clipping planes for bounding box
   GLdouble planeXpos[] = { -1.0, 0.0, 0.0, 2.0 };  // x <= 2
   GLdouble planeXneg[] = { 1.0, 0.0, 0.0, 2.0 };  // x >= -2
   GLdouble planeYpos[] = { 0.0, -1.0, 0.0, 2.0 };  // y <= 2
   GLdouble planeYneg[] = { 0.0, 1.0, 0.0, 2.0 };  // y >= -2
   GLdouble planeZpos[] = { 0.0, 0.0, -1.0, 2.0 };  // z <= 2
   GLdouble planeZneg[] = { 0.0, 0.0, 1.0, 2.0 };  // z >= -2

   glClipPlane(GL_CLIP_PLANE0, planeXpos);
   glClipPlane(GL_CLIP_PLANE1, planeXneg);
   glClipPlane(GL_CLIP_PLANE2, planeYpos);
   glClipPlane(GL_CLIP_PLANE3, planeYneg);
   glClipPlane(GL_CLIP_PLANE4, planeZpos);
   glClipPlane(GL_CLIP_PLANE5, planeZneg);

   glEnable(GL_CLIP_PLANE0);  // +X
   glEnable(GL_CLIP_PLANE1);  // -X
   glEnable(GL_CLIP_PLANE2);  // +Y
   glEnable(GL_CLIP_PLANE3);  // -Y
   glEnable(GL_CLIP_PLANE4);  // +Z
   glEnable(GL_CLIP_PLANE5);  // -Z
   ...
   glBegin(GL_QUADS);
   glVertex3f(-2.0, -2.0, -(-2.0 * jello->a - 2.0 * jello->b + jello->d) / jello->c);  
   glVertex3f(2.0, -2.0, -(2.0 * jello->a - 2.0 * jello->b + jello->d) / jello->c);
   glVertex3f(2.0, 2.0, -(2.0 * jello->a + 2.0 * jello->b + jello->d) / jello->c);
   glVertex3f(-2.0, 2.0, -(-2.0 * jello->a + 2.0 * jello->b + jello->d) / jello->c);
   glEnd();
